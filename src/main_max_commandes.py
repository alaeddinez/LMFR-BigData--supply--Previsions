...
SALES_df = LoadSales('max_comm', 'bq')
#SALES_df = LoadSales('max_nb_comm_hour', 'bq')

df_i = SALES_df.dataframe
df_i.drop(df_i.tail(1).index,inplace=True) 
df_i = df_i.tail(60)

data = df_i.max_nb_comm_hour
data = df_i.max_nb_comm_hour.astype(float)
...
cfg_list = exp_smoothing_configs(seasonal=[30,7])
...
data_future["max_nb_comm_hour"] = np.NaN
...
SCHEMA_PREV = [
  ...
  },
  {
    ...
    "mode": "NULLABLE",
    "name": "max_nb_comm_hour",
    "type": "FLOAT"
  },
  {
    "description": "nombre predit",
    "mode": "NULLABLE",
    "name": "forecast",
    "type": "FLOAT"
  }
...
slack_message = {'text' : 'PREV MAX HOUR COMMAND UPLOADED! :)'}
